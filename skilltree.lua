...
function SkillTree:renderDescriptionBox(skill)
  ...
  _description_box_width = math.max(_description_box_width, _title_text_x + graphics.textWidth(skill.name_trimmed, graphics.FONT_LARGE) + _DESCRIPTION_BORDER*2)
  
  _description_box_width = math.max(_description_box_width, _DESCRIPTION_BORDER + _DESCRIPTION_TEXT_X_PADDING*2 + graphics.textWidth(skill.description_trimmed, graphics.FONT_DEFAULT))
  _description_box_height = math.max(_description_box_height, _DESCRIPTION_BORDER*3 + (_SKILL_SIZE + _BUTTON_BORDER) * _SKILL_SCALE / 2 + graphics.textHeight(skill.description_trimmed, graphics.FONT_DEFAULT))
...
function SkillTree:destroy()
  Cyclone.wmclient.deleteWindow(self.window)
end